...
import { listCards, readCard, search, searchAll, graph, graphAll, overview, exportCards, deleteCard, writeCard, parseCard, stats, optimizeCandidates, optimizeApply, addFeedback, dailyCounts, mergeCards, ensureVault, auditQueue, recycleList, purgeExpired, setCardStatus, restoreCard } from './vault.js'
  ...
  return async function handleApi(req, res) {
    ...
    switch (route) {
      ...
      case '/capture-log': {
        ...
        json(res, 200, { ok: true, available: true, health: getCaptureHealth(), entries: log.slice(0, 100) })
        return
      }
      case '/export': {
        await ensureVault(vaultRoot)
